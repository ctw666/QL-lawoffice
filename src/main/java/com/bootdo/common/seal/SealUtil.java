package com.bootdo.common.seal;
 ...
 * @Description: 印章工具类
 * @Author ben
 * @Date: Created in 15:12 2020-09-14
 */
public abstract class SealUtil {
    ...
    public static BufferedImage buildSeal(SealConfiguration conf) throws Exception {
        ...
        if (conf.getCentreCircle() != null) {
            int x = INIT_BEGIN + borderCircleWidth - conf.getCentreCircle().getWidth();
            int y = INIT_BEGIN + borderCircleHeight - conf.getCentreCircle().getHeight();
            drawCicle(g2d, conf.getCentreCircle(), x, y);
        } else {
            throw new Exception("CentreCircle can not null！");
        }
        ...
        //9 画弧形抬头文字
        if (borderCircleHeight != borderCircleWidth) {
            drawArcFont4Oval(g2d, conf.getBorderCircle(), conf.getTitleFont(), true);
        } else {
            drawArcFont4Circle(g2d, borderCircleHeight, conf.getTitleFont(), true);
        }
